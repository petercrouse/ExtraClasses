...
namespace ExtraClasses.WebUI.FunctionalTests.Controllers.Students
{
    public class Delete : IClassFixture<CustomWebApplicationFactory<Startup>>
    {
        ...
        public async Task GivenId_ReturnsSuccessStatusCode()
        {
            ...
            var response = await _client.DeleteAsync($"/api/students/delete/{validId}");
        ...
        public async Task GivenInvalidId_ReturnsNotFoundStatusCode()
        {
            ...
            var response = await _client.DeleteAsync($"/api/students/delete/{invalidId}");

            Assert.Equal(HttpStatusCode.NotFound, response.StatusCode);
        }

        [Fact]
        public async Task GivenId_ReturnsInternalServerErrorStatusCode_BecauseStudentHasBookings()
        {
            var validId = 1;
